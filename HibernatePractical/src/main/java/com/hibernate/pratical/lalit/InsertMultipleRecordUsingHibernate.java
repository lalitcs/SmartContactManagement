...
public class InsertMultipleRecordUsingHibernate {
    public static void main(String[] args) {
        ...
        String hql = "insert into Student(id, name, company, city, salary) " +
                     "values(:id1, :name1, :company1, :city1, :salary1), " +
                     "(:id2, :name2, :company2, :city2, :salary2), " +
                     "(:id3, :name3, :company3, :city3, :salary3)";

        Query query = s3.createNativeQuery(hql);

        // Set parameter values for each record
        query.setParameter("id1", 68);
